        ...
        if ui.is_rect_visible(rect) {
            ...
            if samples.len() >= 2 {
                ...
                for (i, &sample) in samples.iter().enumerate().take(self.width) {
                    ...
                    // Amplify the sample by 2.0 to make it more visible (since NES audio is typically quiet)
                    let y = center_y - (sample * 2.0 * rect.height() / 2.0);
