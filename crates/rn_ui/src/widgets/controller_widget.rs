...
impl ControllerWidget {
    ...
    fn render_controller(&self, ui: &mut Ui, controller: &ControllerHandlerWrapper, controller_num: u8) {
        ...
        ui.vertical_centered(|ui| {
            ...
            // Action buttons
            ui.horizontal(|ui| {
                if ui.add(interactive_button("B", state.is_button_pressed(ControllerButton::B))).clicked() {
                    toggle_button(controller, controller_num, ControllerButton::B);
                }
                
                ui.add_space(10.0);
                
                if ui.add(interactive_button("A", state.is_button_pressed(ControllerButton::A))).clicked() {
            ...
            ui.horizontal(|ui| {
                if ui.add(interactive_button("SELECT", state.is_button_pressed(ControllerButton::Select))).clicked() {
                    toggle_button(controller, controller_num, ControllerButton::Select);
                }
                
                ui.add_space(10.0);
                
                if ui.add(interactive_button("START", state.is_button_pressed(ControllerButton::Start))).clicked() {
                ...
                .show(ui, |ui| {
                    for button in [
                        ...
                        ui.label(RichText::new(
                            if state.is_button_pressed(button) { "Pressed" } else { "Released" }
                        ).color(
                            if state.is_button_pressed(button) { Color32::GREEN } else { Color32::GRAY }
                        ));
                        ui.end_row();
                    }
                });
