...
impl Assembler {
    ...
    fn split_instruction(&self, input: &str) -> ParseResult<(String, Option<String>)> {
        ...
        let operand = if parts.len() > 1 { Some(parts[1].trim().to_string()) } else { None };
    ...
    fn handle_implied_instruction(&self, instruction: Instruction) -> ParseResult<InstructionMetadata> {
        self.decoder.lookup(instruction, AddressingMode::Implied)
            .map_err(|_| AssembleError::InvalidAddressingMode(
                format!("{instruction} does not support implied mode")
            ))
    }
    ...
    fn handle_label_reference(&self, instruction: Instruction, operand: &str, labels: &HashMap<String, u16>) 
        ...
        let address = labels.get(operand).ok_or_else(|| 
            AssembleError::LabelError(format!("Undefined label: {}", operand))
        )?;
        
        // Use absolute addressing mode for the label
        let metadata = self.decoder.lookup(instruction, AddressingMode::Absolute)
            .map_err(|_| AssembleError::InvalidAddressingMode(
                format!("{instruction} does not support absolute mode for labels")
            ))?;
    ...
    fn parse_instruction(&self, input: &str, labels: Option<&HashMap<String, u16>>) -> ParseResult<InstructionMetadata> {
        ...
        let operand = operand_opt.ok_or_else(|| 
            AssembleError::InvalidSyntax("Missing operand".to_string())
        )?;
        ...
        if let Some(labels_map) = labels {
            ...
            if labels_map.contains_key(&operand) {
                ...
                return self.decoder.lookup(instruction, AddressingMode::Absolute)
                    .map_err(|_| AssembleError::InvalidAddressingMode(
                        format!("{instruction} does not support absolute mode for labels")
                    ));
            }
        ...
        self.decoder.lookup(instruction, addressing_mode).map_err(|_| {
            AssembleError::InvalidAddressingMode(
                format!("{instruction} does not support {addressing_mode:?}")
            )
        })
    }
    ...
    fn encode_instruction(&self, opcode: u8, addressing_mode: AddressingMode, operand_value: u16) -> ParseResult<Vec<u8>> {
        ...
        match addressing_mode {
            ...
            _ => return Err(AssembleError::InvalidAddressingMode(format!(
                "Unsupported addressing mode: {:?}", addressing_mode
            ))),
        }
    ...
    fn collect_labels_and_instructions(&self, program: &str) -> ParseResult<(HashMap<String, u16>, Vec<(String, u16)>)> {
