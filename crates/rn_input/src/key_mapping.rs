...
use crate::{controller_profile::ControllerProfile, error::InputError};
...
impl KeyMappingManager {
    ...
    pub fn to_json(&self) -> Result<String, InputError> {
        serde_json::to_string_pretty(self).map_err(|e| InputError::SerializationError(e.to_string()))
    }

    /// Deserialize from JSON
    pub fn from_json(json_data: &str) -> Result<Self, InputError> {
        serde_json::from_str(json_data).map_err(|e| InputError::DeserializationError(e.to_string()))
    }
...
mod tests {
    ...
    fn test_key_mapping_creation() -> Result<(), InputError> {
        ...
        assert_eq!(
            mapping.profile.get_button_for_key(KeyCode::Z),
        ...
        assert_eq!(
            mapping.profile.get_button_for_key(KeyCode::Tab),
            Some(ControllerButton::Select)
        );
        assert_eq!(
            mapping.profile.get_button_for_key(KeyCode::Enter),
            Some(ControllerButton::Start)
        );
        assert_eq!(
            mapping.profile.get_button_for_key(KeyCode::ArrowUp),
            Some(ControllerButton::Up)
        );
        assert_eq!(
            mapping.profile.get_button_for_key(KeyCode::ArrowDown),
            Some(ControllerButton::Down)
        );
        assert_eq!(
            mapping.profile.get_button_for_key(KeyCode::ArrowLeft),
            Some(ControllerButton::Left)
        );
        assert_eq!(
            mapping.profile.get_button_for_key(KeyCode::ArrowRight),
    ...
    fn test_serialization() -> Result<(), InputError> {
        ...
        assert_eq!(
            deserialized_manager.available_profiles.len(),
            manager.available_profiles.len()
        );
        assert!(deserialized_manager
            .available_profiles
            .iter()
            .any(|p| p.name == "Custom"));
        ...
        assert_eq!(
            custom_profile.get_button_for_key(KeyCode::Space),
            Some(ControllerButton::A)
        );
