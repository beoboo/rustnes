...
Read it with the project's own warning in mind: every number below was true when it was written,
and several were found wrong within days of being recorded. Re-run before quoting.
...
## Appendix: the progress metric, kept as a specimen

The old TODO.md ended with a computed completion percentage. It is preserved here because it is a
better lesson than anything it measured.

**It read 57.5% on a day when the emulator passed every conformance suite that has a verdict to
give** — nestest 8991/8991, instr_test-v5 18/18, ppu_vbl_nmi 11/11, sprite_hit 11/11, apu_test 9/9,
blargg_apu 11/11 and twenty more. The number was honestly computed and completely wrong, because it
counted checkboxes, and roughly 240 of the ~270 open ones were a wishlist: netplay, WebAssembly,
shaders, audio visualisation, several listed twice. The denominator was aspiration.

Two failure modes in one number, both worth recognising elsewhere:

- **The metric counted the wrong things**, so effort spent on the things that mattered barely moved
  it, and the fastest way to raise it would have been to delete ideas.
- **It was computed, so it looked objective.** Nobody re-derives a number that came out of a script.
